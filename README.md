# Vanilla JS Recipe Finder App

As a project for week 3 of school of code our task was to create a recipe finder app with a given API, we decided to create an app to filter recipes through nationality and then it lets the user pick which one he/she likes, so he can see the full recipe.

I decided to use fetch to get the data from the api and we use grid to display our recipes. 

We decided to use modals and multiple event listeners on button and modal background to open/close the modal.

Below you can find the full descriptio of the task.
